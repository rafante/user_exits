...
public class ZValueCondition933 extends ValueFormulaAdapter {
	...
	public BigDecimal overwriteConditionValue(IPricingItemUserExit pricingItem,
		...
		BigDecimal xworki = pricingItem.getSubtotal(PricingCustomizingConstants.ConditionSubtotal.SUBTOTAL_I)
				.getValue();
		// Cópia da variável de trabalho xworkf do R/3
		BigDecimal xworkf = pricingItem.getSubtotal(PricingCustomizingConstants.ConditionSubtotal.SUBTOTAL_F)
				.getValue();
		// Valor da condição do documento atual (KOMV-KWERT)
		BigDecimal xkwert = pricingCondition.getConditionValue().getValue();

		pricingItem.setSubtotal(PricingCustomizingConstants.ConditionSubtotal.SUBTOTAL_I,
				PricingTransactiondataConstants.ZERO);
		pricingItem.setSubtotal(PricingCustomizingConstants.ConditionSubtotal.SUBTOTAL_F,
				PricingTransactiondataConstants.ZERO);
		...
		if (pricingCondition.getConditionTypeName().equals("ZNF3")
				|| pricingCondition.getConditionTypeName().equals("ZNFZ")) {
			...
			String ativar = pricingItem.getAttributeValue("ATIVAR").trim();
			if (ativar.equals("1") || ativar.equals("3")) {
		...
		/*
		 * pricingItem.setSubtotal(PricingCustomizingConstants.ConditionSubtotal.
		 * SUBTOTAL_I,PricingTransactiondataConstants.ZERO); //Limpar variável de
		 * trabalho XWORKF
		 * pricingItem.setSubtotal(PricingCustomizingConstants.ConditionSubtotal.
		 * SUBTOTAL_F,PricingTransactiondataConstants.ZERO);
		 * 
		 * //Verificar se será retornado para o valor da mercadoria o campo //xworki ou
		 * xworkf if(xworki.compareTo(PricingTransactiondataConstants.ZERO) != 0) {
		 * return xworki; } else { return xworkf; }
		 */
		return xkwert;
	}
